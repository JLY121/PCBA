...
class InceptionV3(SimpleNet):
    """Inception V3网络：继承自SimpleNet基类，用于图像分类任务"""
    def __init__(self, num_classes=10, name=None, created_time=None, in_channels=3):
        ...
        self.Conv2d_2a_3x3 = BasicConv2d(32, 32, kernel_size=3, padding=1)  # 保持尺寸
        self.Conv2d_2b_3x3 = BasicConv2d(32, 64, kernel_size=3, padding=1)  # 保持尺寸
        self.maxpool1 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  # 降采样
        self.Conv2d_3b_1x1 = BasicConv2d(64, 80, kernel_size=1)  # 保持尺寸
        self.Conv2d_4a_3x3 = BasicConv2d(80, 192, kernel_size=3, padding=1)  # 保持尺寸
        self.maxpool2 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  # 降采样

        # Inception A模块（35x35特征图）
        self.Mixed_5b = InceptionA(192, pool_features=32)  # 256
        self.Mixed_5c = InceptionA(256, pool_features=64)  # 288
        self.Mixed_5d = InceptionA(288, pool_features=64)  # 288

        # Inception B模块（降采样：35x35 -> 17x17）
        self.Mixed_6a = InceptionB(288)  # 768

        # Inception C模块（17x17特征图）
        self.Mixed_6b = InceptionC(768, channels_7x7=128)  # 768
        self.Mixed_6c = InceptionC(768, channels_7x7=160)  # 768
        self.Mixed_6d = InceptionC(768, channels_7x7=160)  # 768
        self.Mixed_6e = InceptionC(768, channels_7x7=192)  # 768

        # Inception D模块（降采样：17x17 -> 8x8）
        self.Mixed_7a = InceptionD(768)  # 1280

        # Inception E模块（8x8特征图）
        self.Mixed_7b = InceptionE(1280)  # 2048
        self.Mixed_7c = InceptionE(2048)  # 2048
    ...
    def forward(self, x):
        # 输入预处理（期望输入尺寸为299x299，但这里适配较小的输入）
        # N x 3 x 299 x 299 (或适应性调整)
...
def InceptionV3Net(name=None, created_time=None, num_classes=10, in_channels=3):
    """创建Inception V3模型实例"""
...
def SupConInceptionV3(name=None, created_time=None, num_classes=10, in_channels=3):
    """创建用于对比学习的Inception V3 Encoder模型实例"""
